...
import type { SocketEventDataEntrySyncStringUpdateChange } from '@thebcms/selfhosted-backend/socket/events/entry-sync';
...
export function patienceDiffMerge(
    diffs: SocketEventDataEntrySyncStringUpdateChange[],
    input: string,
): string {
    ...
    for (let i = 0; i < diffs.length; i++) {
        const diff = diffs[i];
        if (diff.ai && diff.av) {
            output.splice(diff.ai, 0, diff.av);
        }
    }
    return output.join('');
}

export function patienceDiffToSocketEvent(lines: PatienceDiffLine[]) {
    const output: SocketEventDataEntrySyncStringUpdateChange[] = [];
